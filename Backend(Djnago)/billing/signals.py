...
from .utils import schedule_expire_sms_task, remove_existing_task
...
def handle_subscription_save(sender, instance: OrganizationSubscription, created, **kwargs):
    ...
    if instance.status == 'active' and instance.end_date and instance.end_date > now:
        ...
        is_expiring_soon = instance.end_date <= (now + timezone.timedelta(minutes=5))
        # direct update without triggering signal
        OrganizationSubscription.objects.filter(id=instance.id).update(
